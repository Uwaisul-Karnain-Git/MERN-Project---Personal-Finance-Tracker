...
({ value: initialValue, row, column, updateRecord, editable}) => {
  ...
  return (
    <div onClick={() => editable && setIsEditing(true)} style={{ cursor: editable ? "pointer" : "default" }}>
      { isEditing ? 
        <input value={value} onChange={e => setValue(e.target.value)} autoFocus onBlur={handleOnBlur} 
          style={{width: "100%"}} /> 
...
const FinancialRecordList = () => {
  ...
  const columns: Array<Column<FinancialRecord>> = useMemo(() => [
    ...
    },
    {
      ...
      id: "delete",
      Cell: ({row}) => (
        <button className='button' onClick={() => deleteRecord(row.original._id ?? "")}>Delete</button>
      )
    }
  ], [records]);
